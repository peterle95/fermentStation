...
Camera capture in `camera.ts` is capability-gated. Android process death can return a Capacitor `appRestoredResult`; `listenForRestoredCameraPhoto` lets `App` recover the pending photo flow. A canceled picker, denied permission, unavailable plugin, or failed conversion leaves the pending batch and timeline unchanged; browser/desktop use file inputs when native capture is unavailable. If a shared `photoRef` is missing, unsafe, or unreadable, hydration fails the whole snapshot into a visible `problem` status rather than dropping the photo or applying partial state. `native-transfer.ts` similarly prefers native archive pick/share and falls back to browser download/upload; canceled transfer returns no bytes and does not mutate local state.
