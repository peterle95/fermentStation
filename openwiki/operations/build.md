...
Capacitor sync copies the Vite web bundle and plugin metadata into Android; Android requires Android Studio JDK/SDK and uses `android/` Gradle configuration, `AndroidManifest.xml`, Java plugins, and generated assets. Tauri uses `src-tauri/tauri.conf.json`, Rust `src-tauri/src/main.rs`, and the PowerShell wrapper scripts; desktop bundling is enabled with `icons/icon.ico`, and Rust/MSVC tooling is required. Web runs in the browser, Android supplies Capacitor plugins/SAF, and desktop supplies Tauri commands/filesystem behavior.
