...
`src/domain/batches.ts` owns the batch aggregate. `App.tsx` calls `createBatch` from the start-batch flow, applies returned states through `saveBatches`, and calls `updateBatchDates` during initialization and every batch save. That wrapper runs `discardExpiredBatches` and date projection, so UI-triggered saves can both expire seven-day trash and promote finished active batches before browser/shared persistence. `Batch` stores a cloned `profileSnapshot`, numeric inputs, calculated values, status, finish date, recurring checks, timeline entries, and timeline trash; `BatchState` separates active records from trashed batches.

## Lifecycle and invariants

`createBatch` snapshots the profile, fills defaults, rejects non-finite/negative inputs, creates check due dates, calculates outputs, and optionally projects today’s status. Automatic date promotion uses `changeBatchStatus` internally but does not itself append a status timeline entry; explicit timeline status entries are the separate path handled by timeline recomputation. Manual UI status actions likewise pass through the domain transition helper, while later `updateBatchDates` can derive readiness from the finish date. Statuses are `active`, `ready`, and `to-fridge`. `changeBatchStatus` records `checksPausedAt` when leaving active and shifts all due dates by paused calendar days when reactivated. `updateBatchForDate` promotes an active batch whose finish date has arrived. Checks can be added, renamed, adjusted, completed, and queried by `dueBatchChecks`; non-active batches reject check mutations. A completed check updates `lastCompletedDate`, advances `nextDueDate`, and emits a check timeline entry; `dueBatchChecks` sorts by due date/name/id and excludes paused batches.
...
`setBatchInput` and `overrideBatchCalculation` enforce known names and nonnegative values. Calculations use the profile snapshot, so later profile edits cannot silently change an existing batch. pH entries go through `addPhReading`/`updatePhReading`, allowing two decimal places; `phZoneLabel` uses snapshot zones and `phWarning` flags values outside 0–14. The UI's measurement panel reads the latest temperature timeline entry (not a profile input), compares it with the snapshot's `temperatureMinC`/`temperatureMaxC`, and compares pH with the aggregate minimum/maximum zone bounds for its position and alert state. Calendar and Today navigation consume the derived events described in [calendar and upcoming navigation](../workflows/calendar.md).
